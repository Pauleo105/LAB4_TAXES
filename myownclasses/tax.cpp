...
namespace taxes {
    ...
    std::ostream& Budget::print(std::ostream& c) const{   //орет на конст после принта
        c << "Full name: " << surname+' ' << name+' ' << lastname << ", workplace: " << workplace << ", post: " << post << "\n";
        c << "List of payments: " << std::endl;
        int i = 1;
        for (mine::list<Payment>::Iterator tmp = ptr.begin(); tmp != nullptr; tmp++, i++) c << i << ") " << (*tmp).value;
        if (ptr.isEmpty()) c << "Empty";
        c << std::endl;
        return c;
    };

    std::pair<mine::list<Payment>::Iterator, mine::list<Payment>::Iterator> Budget::getIt() {
        std::pair<mine::list<Payment>::Iterator, mine::list<Payment>::Iterator> tmp;
        tmp.first = ptr.begin();
        tmp.second = ptr.end();
        return tmp;
    }
    ...
    unsigned int Budget::getGain(std::string typ) const {
        unsigned int gain = 0;
        for (mine::list<Payment>::Iterator tmp = ptr.begin(); tmp != ptr.end(); tmp++) {
            if (typ == (*tmp).value.getType()) gain += (*tmp).value.getSum();
        }
        return gain;
    }
    ...
    double Table::counttax(unsigned int num) {
        ...
        if (ptr == nullptr) std::cout << "Person with this private number doesn't exist!" << std::endl;
            ...
            for (std::multimap <unsigned int, taxes::Budget*>::iterator tmp = pp.first; tmp != pp.second; tmp++) {
                for (mine::list<Payment>::Iterator tmp1 = tmp->second->getIt().first; tmp1 != nullptr; tmp1++) {
                    tax += (*tmp1).value.getSum()*0.13;
                }
            }
        }
        return tax;
